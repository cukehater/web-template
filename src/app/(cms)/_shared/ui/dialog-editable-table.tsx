...
import { DialogEditableTableProps, EditableTableColumn } from '../model'
import PageTopTitle from './page-top-title'

export default function DialogEditableTable<T extends Record<string, any>>({
  data,
  columns,
  dialogColumns,
  onSave,
  ...
  const renderFormField = (column: EditableTableColumn<T>) => {
    const value = editData[column.key]
        ...
        <DialogContent>
          ...
          <form className='space-y-4 mt-2'>
            {dialogColumns?.map(column => (
              <div key={String(column.key)}>{renderFormField(column)}</div>
            ))}
          </form>
