...
import { apiDelete, apiGet, apiPut } from '@cms/shared/api'
import { ALERT_MESSAGES, errorToast, infoToast, successToast } from '@cms/shared/lib'
...
export default function GalleryTable({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await apiGet<GalleryResponseType>(
          `/api/gallery?page=${currentPage}&limit=${currentLimit}`
        )

        setData(data?.data as Gallery[])
        setPaginationInfo(data?.pagination as PaginationType)
      } catch {
